...
export interface InputChoiceOption {
  id: string;
  label: string;
  hint?: string;
  badge?: string;
...
interface ChatInputProps {
  ...
  choiceOptions?: InputChoiceOption[];
  onChoiceSelect?: (option: InputChoiceOption) => void;
  stepLabel?: string;
...
const isTextualFile = (file: File): boolean => {
  ...
  const textualExtensions = ["txt", "md", "py", "js", "ts", "jsx", "tsx", "html", "css", "json", "yaml", "yml", "csv"];
  const extension = file.name.split(".").pop()?.toLowerCase() || "";
  return textualTypes.some((type) => file.type.toLowerCase().startsWith(type)) || textualExtensions.includes(extension);
};
...
const FilePreviewCard: React.FC<{ file: FileWithPreview; onRemove: (id: string) => void }> = ({ file, onRemove }) => {
  ...
  return (
    <div className={cn("relative group bg-zinc-700 border w-fit border-zinc-600 rounded-lg size-[125px] shadow-md flex-shrink-0 overflow-hidden", isImage ? "p-0" : "p-3")}>
      ...
      <Button size="icon" variant="outline" className="absolute top-1 right-1 h-6 w-6 p-0 opacity-0 group-hover:opacity-100" onClick={() => onRemove(file.id)}>
        <X className="h-4 w-4" />
      </Button>
    </div>
  );
    ...
    <div className="bg-zinc-700 border border-zinc-600 relative rounded-lg p-3 size-[125px] shadow-md flex-shrink-0 overflow-hidden">
      ...
      <div className="group absolute flex justify-start items-end p-2 inset-0 bg-gradient-to-b to-[#30302E] from-transparent overflow-hidden">
        ...
        <div className="group-hover:opacity-100 opacity-0 transition-opacity duration-300 flex items-center gap-0.5 absolute top-2 right-2">
          <Button size="icon" variant="outline" className="size-6" onClick={() => navigator.clipboard.writeText(content.content)} title="Copy content">
            <Copy className="h-3 w-3" />
          </Button>
          <Button size="icon" variant="outline" className="size-6" onClick={() => onRemove(content.id)} title="Remove content">
            <X className="h-3 w-3" />
          </Button>
        </div>
      </div>
...
const ModelSelectorDropdown: React.FC<{ models: ModelOption[]; selectedModel: string; onModelChange: (modelId: string) => void }> = ({
  models,
  selectedModel,
  onModelChange,
}) => {
        ...
        <div className="absolute bottom-full right-0 mb-2 w-72 bg-zinc-800 border border-zinc-700 rounded-lg shadow-xl z-20 p-2">
          {models.map((model) => (
            <button
              key={model.id}
              type="button"
              className={cn("w-full text-left p-2.5 rounded-md hover:bg-zinc-700 transition-colors flex items-center justify-between", model.id === selectedModel && "bg-zinc-700")}
              onClick={() => {
            ...
            >
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {model.badge ? <span className="px-1.5 py-0.5 text-xs bg-blue-500/20 text-blue-300 rounded">{model.badge}</span> : null}
                </div>
                <p className="text-xs text-zinc-400 mt-0.5">{model.description}</p>
              </div>
              {model.id === selectedModel ? <Check className="h-4 w-4 text-blue-400 flex-shrink-0" /> : null}
            </button>
          ))}
        </div>
      ) : null}
...
export function ClaudeChatInput({
  ...
  const handleFileSelect = useCallback(
    (selectedFiles: FileList | null) => {
      if (!selectedFiles || !showAttachButton) return;
    ...
    [files.length, maxFileSize, maxFiles, showAttachButton],
  );
    ...
    (e: React.ClipboardEvent<HTMLTextAreaElement>) => {
      ...
      if (textData && textData.length > PASTE_THRESHOLD && pastedContent.length < 5) {
        ...
        setPastedContent((prev) => [
          ...prev,
          { id: newId(), content: textData, timestamp: new Date(), wordCount: textData.split(/\s+/).filter(Boolean).length },
        ]);
      }
    },
    [files.length, handleFileSelect, maxFiles, message, pastedContent.length, setMessage, showAttachButton],
  );

  const handleSend = useCallback(() => {
    if (disabled || (!message.trim() && files.length === 0 && pastedContent.length === 0)) return;
    onSendMessage?.(message, files, pastedContent);
    if (value === undefined) setInternalMessage("");
    files.forEach((file) => {
  ...
  }, [disabled, files, message, onSendMessage, pastedContent, value]);
  ...
  return (
    <div
      ...
      onDragOver={showAttachButton ? (e) => { e.preventDefault(); setIsDragging(true); } : undefined}
      onDragLeave={showAttachButton ? (e) => { e.preventDefault(); setIsDragging(false); } : undefined}
      onDrop={showAttachButton ? (e) => { e.preventDefault(); setIsDragging(false); handleFileSelect(e.dataTransfer.files); } : undefined}
    >
      {isDragging ? (
      ...
      <div className="bg-[#30302E] border border-zinc-700 rounded-xl shadow-lg min-h-[150px] flex flex-col">
        {(stepLabel || stepDescription) && (
          <div className="px-4 pt-4 pb-1 border-b border-zinc-700/70">
            {stepLabel ? <p className="text-[11px] uppercase tracking-[0.14em] text-amber-500/90 font-medium">{stepLabel}</p> : null}
            {stepDescription ? <p className="text-sm text-zinc-300 mt-1">{stepDescription}</p> : null}
          </div>
        )}
        ...
        <textarea
          ...
          disabled={disabled}
          className="flex-1 min-h-[100px] w-full p-4 focus:outline-none max-h-[120px] resize-none border-0 bg-transparent text-zinc-100 placeholder:text-zinc-500 text-sm sm:text-base"
          rows={1}
        />

        {choiceOptions && choiceOptions.length > 0 ? (
          <div className="px-3 pb-2 border-t border-zinc-700/70 pt-3">
            <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto">
              {choiceOptions.map((option) => (
                <button
                  ...
                  className="inline-flex flex-col items-start rounded-lg border border-zinc-600 bg-zinc-800/80 px-3 py-2 text-left transition hover:border-amber-600/60 hover:bg-zinc-700 disabled:opacity-50"
                >
                  <span className="text-sm font-medium text-zinc-100">{option.label}</span>
                  {option.hint ? <span className="text-[11px] text-zinc-400 mt-0.5">{option.hint}</span> : null}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 justify-between w-full px-3 pb-1.5">
          <div className="flex items-center gap-2">
            ...
            {showOptionsButton ? (
              <Button size="icon" variant="ghost" className="h-9 w-9 p-0 text-zinc-400 hover:text-zinc-200 hover:bg-zinc-700 flex-shrink-0" disabled={disabled} title="Optionen">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className={cn(
                ...
                canSend ? "bg-amber-600 hover:bg-amber-700 text-white" : "bg-zinc-700 text-zinc-500 cursor-not-allowed",
              )}
              onClick={handleSend}
              disabled={!canSend}
              title="Senden"
