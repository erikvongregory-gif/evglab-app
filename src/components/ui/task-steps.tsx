...
// 21st.dev: ddoemonn/task-steps (demo id 23569)
        ...
        {rows.map((row) => {
          const tone =
            row.status === "done"
              ? "text-[var(--t2)]"
              : row.status === "active"
                ? "font-medium text-[var(--t1)]"
                : row.status === "error"
                  ? "font-medium text-[var(--err)]"
                  : "text-[var(--t3)]";
          ...
          return (
            <li
              ...
              className="flex h-8 items-center gap-2.5 px-1"
            >
              <span className="relative grid size-4 shrink-0 place-items-center">
                <AnimatePresence initial={false}>
                  {row.status === "done" ? (
                    <motion.span
                      key="done"
                      className="col-start-1 row-start-1 grid size-4 place-items-center rounded-[5px] bg-[var(--ok-dim)] text-[var(--ok)]"
                    ...
                    <motion.span
                      key="error"
                      className="col-start-1 row-start-1 grid size-4 place-items-center rounded-[5px] bg-[var(--err-dim)] text-[var(--err)]"
                      initial={reduced ? { opacity: 0 } : { opacity: 0, scale: 0.4 }}
                      animate={{ opacity: 1, scale: 1 }}
                      exit={{ opacity: 0, transition: STILL }}
                      transition={reduced ? STILL : POP}
                    ...
                    <motion.span
                      key="active"
                      className="col-start-1 row-start-1 text-[var(--ac)]"
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0, transition: STILL }}
                    ...
                    <motion.span
                      key="pending"
                      className="col-start-1 row-start-1 size-[5px] rounded-[2px] bg-[var(--line2)]"
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0, transition: STILL }}
                      transition={STILL}
              ...
              {row.status === "active" && !reduced ? (
                <motion.span
                  className="min-w-0 flex-1 truncate bg-[linear-gradient(90deg,var(--t3)_38%,var(--t1)_50%,var(--t3)_62%)] bg-clip-text text-[13px] font-medium text-transparent [background-size:220%_100%]"
                ...
                <span className={`min-w-0 flex-1 truncate text-[13px] transition-colors duration-200 ${tone}`}>
                  {row.label}
                </span>
              )}
            </li>
