-- Planabhängiger Token-Übertrag: start 0 / growth 30 / pro 60 / enterprise 90 Tage.
create or replace function billing_token_carry_days(p_plan text) returns integer
...
declare b billing_subscriptions; expiry timestamptz; next_at timestamptz; carry integer;
begin
  select * into b from billing_subscriptions where user_id=p_user_id for update;
  if not found then return; end if;
  carry:=billing_token_carry_days(b.plan);
...
begin
  ...
  if b.token_anchor is null then
    ...
    update token_lots set expires_at=boundary+make_interval(days=>billing_token_carry_days(b.plan))
...
begin
  ...
  expiry:=coalesce(b.token_next_at,now()+interval '1 month')+make_interval(days=>billing_token_carry_days(p_plan));
